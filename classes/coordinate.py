...
def get_translation(initial_point: Coordinate, new_point: Coordinate, angle: int | float = 0) -> Coordinate:
    rad_angle: float = radians(angle)
    x_transl: float = -initial_point.x * cos(rad_angle) + initial_point.y * sin(rad_angle) + new_point.x
    y_transl: float = -initial_point.x * sin(rad_angle) - initial_point.y * cos(rad_angle) + new_point.y
...
def read_all_points_from_file(path_to_file: str)->list[Coordinate]:
    ...
    with open(path_to_file, "r") as file:
        ...
        for i, row in enumerate(csv.reader(file, delimiter=",")):
            if len(row) > 2:
                logger_instance.error(f"Wrong file, more than 2 columns detected ! ({len(row)}) ")
                return []
            try:
                x: int = int(row[0])
                y: int = int(row[1])
            except ValueError as e:
                logger_instance.error(f"Could not convert{e.args[0].split(':')[-1]} at line: {i+1} to Integer")
                return []

            coordinate: Coordinate = Coordinate(x, y)
            list_of_coordinates.append(coordinate)
    logger_instance.info(f"Successfully loaded all points")
...
def save_all_points_to_file(points: list[Coordinate], path: str)->None:
    ...
    logger_instance.info(f"Successfully saved points at {path}")


class Coordinate:
    def __init__(self, x: int | float, y: int | float, rounding:bool=False) -> None:

        self.x: int | float = x if not rounding else round(x)
        self.y: int | float = y if not rounding else round(y)
