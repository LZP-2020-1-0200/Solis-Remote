...
class GUI(Frame):
    ...
    def __init__(self, parent:Misc) -> None:
        ...
        Button(self, text="Go to anchor 1", font=TEXT_FONT,
            command=lambda:self._go_to_anchor(0)).grid(row=0,column=0)
        Button(self, text="Go to anchor 2", font=TEXT_FONT,
            command=lambda:self._go_to_anchor(1)).grid(row=0,column=1)
        Button(self, text="Go to anchor 3", font=TEXT_FONT,
            command=lambda:self._go_to_anchor(2)).grid(row=0,column=2)
